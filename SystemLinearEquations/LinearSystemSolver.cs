...
namespace SystemLinearEquations
{
    ...
    {
        public static double[] SolveSystem(Matrix matrix, double[] b)
        {
            // this block of conditional statements are based on optimizing getting a solution based on time to execute
            if (matrix.Dimensions.Column <= 0)
            {
                throw new ArgumentException("Dimension of" + matrix.Dimensions.Column + "not allowed");
            }
            // am going to have to change this bound
            else if (matrix.Dimensions.Column < 24)
            {
