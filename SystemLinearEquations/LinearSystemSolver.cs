...
namespace SystemLinearEquations
{
    ...
    {
        public static double[] SolveSystem(Matrix A, double[] b)
        {
            // am going to have to change this boundary
            if (A.Dimensions.Column < 24)
            {
                return new CramersMethodModified().Solve(A, b);
            }
            else
            {
                return new ConjugateTransposeMethods().Solve(A, b);
            }
        }
